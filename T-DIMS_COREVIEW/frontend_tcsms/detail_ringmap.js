(function (global) {
  ...
  function normalizeRow(row, index) {
    ...
    const sortRing = Number.isFinite(Number(ring)) ? Number(ring) : index + 1;
  ...
  function renderProjectRows(map, rows, completedRing, options) {
  ...
  async function syncExistingMap(config) {
    ...
    toggleEmptyOverlay(config.container || config.map.getContainer(), config.projectKey, !state.rows.length);
    if (!state.rows.length) return state;
    clearDataLayers(config.map, [config.satelliteLayer, config.mapLayer]);
    renderProjectRows(config.map, state.rows, state.completedRing, config);
    return state;
  }
  ...
  async function initStandaloneMap(config) {
    ...
    const state = await fetchProjectState(config.projectKey);
    toggleEmptyOverlay(container, config.projectKey, !state.rows.length);
    if (state.rows.length) {
      renderProjectRows(map, state.rows, state.completedRing, config);
    }
